      ...
      <div className="container mx-auto px-4 py-8 max-w-3xl">
        <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
          ...
          <div className="flex border-b border-secondary-100">
            <div
              className={`flex-1 py-4 text-center text-sm font-semibold ${step >= 1 ? "text-primary-600 border-b-2 border-primary-600" : "text-secondary-400"}`}
            >
              1. Specialist
            </div>
            <div
              className={`flex-1 py-4 text-center text-sm font-semibold ${step >= 2 ? "text-primary-600 border-b-2 border-primary-600" : "text-secondary-400"}`}
            >
              2. Doctor
            </div>
            <div
              className={`flex-1 py-4 text-center text-sm font-semibold ${step >= 3 ? "text-primary-600 border-b-2 border-primary-600" : "text-secondary-400"}`}
            >
              3. Time
            </div>
            <div
              className={`flex-1 py-4 text-center text-sm font-semibold ${step >= 4 ? "text-primary-600 border-b-2 border-primary-600" : "text-secondary-400"}`}
            >
              4. Confirm
            </div>
          </div>
            ...
            {step === 1 && (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 animate-in fade-in slide-in-from-right-4 duration-300">
                {specializations.map((spec) => (
                  ...
                  >
                    {spec}
                    <span className="opacity-0 group-hover:opacity-100 transition-opacity text-primary-600">
                      →
                    </span>
                  </button>
                ))}
              ...
              <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
                ...
                {doctors.length > 0 ? (
                  ...
                  <div className="text-center py-8 text-secondary-500">
                    ...
                    <button
                      onClick={() => setStep(1)}
                      className="block mx-auto mt-4 text-primary-600 hover:underline"
                    >
                      Go back
                    </button>
                  </div>
            ...
            {step === 4 && (
              <div className="text-center space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                ...
                <div className="flex justify-between pt-4 gap-4">
                  <button
                    onClick={() => setStep(3)}
                    className="flex-1 py-3 rounded-xl border border-secondary-200 font-semibold text-secondary-700 hover:bg-secondary-50"
                  >
                    Back
                  </button>
                  <button
